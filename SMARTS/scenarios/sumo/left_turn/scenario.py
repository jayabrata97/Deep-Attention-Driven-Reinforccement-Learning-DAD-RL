...
from smarts.sstudio.genscenario import gen_scenario, gen_missions, gen_traffic
...
for seed in np.random.choice(1000, 20, replace=False):
    ...
    west_east_flow = [Flow(route=Route(begin=("edge-west-WE", i, "random"), end=(f"edge-east-WE", i, "random")),
                           ...
                           repeat_route=True) for i in range(2)]
    ...
    traffic[str(seed)] = Traffic(flows = west_east_flow + east_west_flow + turn_right_flow)
...
route = Route(begin=("edge-south-SN", 1, 40), end=("edge-west-EW", 0, "max"))
ego_missions = [
    Mission(
        route=route,
