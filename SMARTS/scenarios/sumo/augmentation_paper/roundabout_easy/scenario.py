    ...
    for i, start in enumerate(start_routes):
        for end in end_routes:
            if end.split('-')[0] != start.split('-')[0]:
                flows.append(Flow(route=Route(begin=('edge-'+start, 0, "random"), end=('edge-'+end, 0, "random")), rate=80, actors=actors, repeat_route=True))
    flows.append(Flow(route=Route(begin=('edge-east-EN', 1, "random"), end=('edge-south-NS', 0, "random")), rate=100, actors=actors, repeat_route=True))
...
route = Route(begin=("edge-east-EW", 0, 1), end=("edge-north-SN", 0, "max"))
ego_missions = [
    Mission(
        route=route,
