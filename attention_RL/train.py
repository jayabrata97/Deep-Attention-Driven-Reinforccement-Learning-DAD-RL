#version 2 refers to updated reward function
import os 
import numpy as np
import torch
import random
...
from feature_extractor import CombinedExtractor_PPO_v2, CombinedExtractor_v3, StateExtractor_v3
...
def main():
    ...
    model_list = ['SAC_v5', 'Attention_SAC_v5', 'PPO_v5', 'Attention_PPO_v5']
    ...
    def make_env(scenario, seed):
        env = wrapped_env(scenario, reward_func, include_context, interface_version, wrapper_version, seed, True, max_speed=max_speed)
    ...
    vec_env = SubprocVecEnv([lambda: make_env(x,y) for (x,y) in zip(training_scenarios_scaled,train_seeds)], 
    ...
    if checkpoint_path is None:
        ...
        if include_context:
            if Attention:
                policy_kwargs = dict(features_extractor_class=CombinedExtractor_v3,
            ...
            if Attention:
                policy_kwargs = dict(features_extractor_class=StateExtractor_v3, 
                                    features_extractor_kwargs=dict(pos_enc=include_pe))
    ...
    class BenchmarkCallback(BaseCallback):
        ...
        def _on_step(self) -> bool:
            if self.num_timesteps>0 and self.num_timesteps%TIMESTEPS==0:
                ...
                benchmark_logger(log_dir, scenarios, scenes, model_path, Eval_seed, N_eval_envs, reward_func, include_context, interface_version, wrapper_version, eps, i, TIMESTEPS, max_speed=max_speed, modify_waypoints=modify_waypoints)
    ...
    model.learn(total_timesteps=TRAINING_TIMESTEPS, reset_num_timesteps=False, callback=CallbackList([checkpoint_callback, benchmark_callback]), progress_bar=True, tb_log_name='train')

    vec_env.close()
    # eval_env.close() # TO DO: CHECK
    model.env.close()
    del model
    # del data # TO DO: CHECK
