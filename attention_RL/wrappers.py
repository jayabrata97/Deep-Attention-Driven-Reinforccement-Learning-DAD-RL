...
from data_utils import get_ego_state_vector_v3, get_sv_state_vectors_v3, context_observation_adapter_v2
...
def sparse_reward(road_map, scen, env_obs, done):
    ...
    reward = R_crash*(1+0.1*speed) + R_off_road*(1+ 0.1*speed) + 3*R_goal + 2*R_progress

    return reward

def reward_type6_v8(road_map, scen, env_obs, done, slow_counter):
    
    if (env_obs['ego_vehicle_state']['lane_id']=="off_lane") or (env_obs['events']['off_road']==True):
        speed_limit = 0
    else:
        speed_limit = road_map.lane_by_id(env_obs['ego_vehicle_state']['lane_id']).speed_limit
    ...
    if speed_limit==0:  #to avoid dividing by zero
        ...
        R_speed = (speed/speed_limit) if (R_off_route==0 and R_wrong_way==0) else 0
    else:
    ...
    reward = R_crash + R_off_road + 3*R_goal + 2*R_progress + 0.01*R_speed + 0.01*R_off_route + 0.01*R_wrong_way + 0.2*R_slow
...
class StateWrapper(gym.Wrapper):
    def __init__(self, env, reward_func, interface_version, wrapper_version, max_speed):
        super().__init__(env)
        
        self.interface_version = interface_version
        if interface_version==1 or interface_version==2:
            self.action_space = gym.spaces.Box(low=np.array([0.,0.,-1.], dtype=np.float32), high=np.array([1.,1.,1.], dtype=np.float32), shape=(3,))
        ...
        self.observation_space = gym.spaces.Dict({
            'sv_states': gym.spaces.Box(shape=self.sv_shape, low=-np.inf, high=np.inf),
            'ev_states': gym.spaces.Box(shape=self.ego_shape, low=-np.inf, high=np.inf)
        })
    ...
    def step(self, action):
        ...
        if self.reward_func == 'type1':
            ######################### Reward Adapter 1 #############################
            reward = 0.01*progress + goal + crash + off_road + 0.01*off_route + 0.01*wrong_way 
        elif self.reward_func == 'type2':
            ######################### Reward Adapter 2 ###########################
            crash = crash*(1 + 0.1*speed)
            reward = 0.1*progress + goal + crash + off_road + 0.01*off_route + 0.01*wrong_way
        elif self.reward_func == 'type3':
            ######################### Reward Adapter 3 ###########################
            crash = crash*(1 + 0.1*speed)
            progress = progress if (off_route==0 and wrong_way==0) else 0
            reward = 0.1*progress + goal + crash + off_road + 0.01*wrong_way
        elif self.reward_func == 'type4':
            ######################### Reward Adapter 4 ###########################
            crash =  crash*(1 + 0.1*speed)
            off_road = off_road*(1 + 0.1*speed)
            progress = progress if (off_route==0 and wrong_way==0) else 0
            reward = 0.02*progress + goal + crash + off_road
        elif self.reward_func == 'type5':
            ######################### Reward Adapter 5 ###########################
            not_moving =  -1 if env_obs['events']['not_moving'] else 0
            crash =  crash*(1 + 0.1*speed)
            off_road = off_road*(1 + 0.1*speed)
            progress = progress if (off_route==0 and wrong_way==0) else 0
            reward = 0.02*progress + goal + crash + off_road + 0.01*not_moving
        elif self.reward_func == 'type6_v8':
            ######################### Reward Adapter 6 ###########################
            reward, self.checkpoint = reward_type6_v8(self.road_map, self.scen, env_obs, dones, self.slow_counter)
        ...
        ego_state_vector, self.ev_history_queue = get_ego_state_vector_v3(ev_history_queue=self.ev_history_queue, env_obs=env_obs, road_map=self.road_map)

        ################################# Surrounding vehicle observation ##################################
        new_sv = env_obs['neighborhood_vehicle_states']
        self.sv_history_queue.append(new_sv)
        sv_state_vectors = get_sv_state_vectors_v3(self.sv_history_queue, env_obs=env_obs)
        ...
        info['RewardComponents'] = {'speed': speed, 'progress':progress, 'goal':abs(env_obs['events']['reached_goal']), 
                                    'crash':abs(env_obs['events']['collisions']), 'off_road':abs(env_obs['events']['off_road']), 
                                    'off_route':abs(off_route), 'wrong_way':abs(wrong_way), 'not_moving':abs(not_moving)}
...
class ObsWrapper(gym.Wrapper):
    def __init__(self, env, reward_func, interface_version, wrapper_version, max_speed):
        ...
        self.observation_space = gym.spaces.Dict({
            'sv_states': gym.spaces.Box(shape=self.sv_shape, low=np.float32(-np.inf), high=np.float32(np.inf)),
            'ev_states': gym.spaces.Box(shape=self.ego_shape, low=np.float32(-np.inf), high=np.float32(np.inf)),
            'context': gym.spaces.Box(shape=self.context_shape, low=0, high=255, dtype=np.uint8), # changed the shape from (4, 256, 256) to (8, 256, 256)
        })
    ...
    def step(self, action):
        ...
        ################################# Context Observation ###############################
        context_states, self.goal_lane_index = context_observation_adapter_v2(env_obs, self.goal_lane_index, self.modify_waypoints)
    ...
    def reset(self):
        ...
        init_ego_vector[0] = 1   #padding
        self.ev_history_queue = deque([init_ego_vector]*21, maxlen=21)
        self.sv_history_queue = deque([{}]*21, maxlen=21)   #current timstep + last 2s of info
