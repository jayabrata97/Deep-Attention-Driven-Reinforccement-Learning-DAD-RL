...
def get_ego_state_vector_v3(ev_history_queue, env_obs, road_map) -> np.array:
    ...
    ego_state_vector = np.array(
        [ego_state_vector.reshape((ego_state_vector.size))]
    )  # convert (20,) to (1,20)
...
def get_sv_state_vectors_v3(sv_history_queue, env_obs) -> np.array:
    ...
    for (
        ...
        sv_dict
    ):  # making a dictionary with the vehicle ids and their relative distances from the ego based on current timestep
        curr_state = sv_dict[id][-1, :].squeeze()
        sv_pos = np.array(curr_state[1:3])  # first feature is padding
        if (
            curr_state[0] == 1
        ):  # when the vehicle is not present in the current timestep
            dist = np.inf
        elif curr_state[0] == 0:
            dist = np.linalg.norm(sv_pos - ego_pos)
        dist_dict[id] = dist

    sorted_dist_dict = dict(
        sorted(dist_dict.items(), key=lambda i: i[1])
    )  # sorting the dict based on the values
    filtered_ids = list(sorted_dist_dict.keys())[
        :6
    ]  # clipping nearest 6 vehicles (if <=6 then no change)
    filtered_sv_dict = {}
    for id in filtered_ids:
        filtered_sv_dict[id] = sv_dict[
            id
        ]  # obtain the sv_states based on the clipped ids
...
def context_observation_adapter_v2(
    env_obs, goal_lane_index, modify_waypoints: bool
) -> np.array:
    """
    ...
    if modify_waypoints == False:
        ...
        ego_lane = env_obs["ego_vehicle_state"][
            "lane_index"
        ]  # waypoints only collected for the lane that agent is following
        waypoints_global = env_obs["waypoint_paths"]["position"][
            ego_lane, :50, :
        ]  # in global coordinates, probably 12 lanes.
        ...
        if dist <= 30:
            if goal_lane_index == -1:  # find goal_lane
                ...
                waypoints_global[:50] = env_obs["waypoint_paths"]["position"][
                    goal_lane_index, :50
                ]
        ...
        if len(context_observation_adapter_v2.cons_wp) < 2:
            ...
            context_observation_adapter_v2.cons_wp[0] = (
                context_observation_adapter_v2.cons_wp[1]
            )
        ...
        if (
            len(context_observation_adapter_v2.cons_wp) == 2
            and np.sum(waypoints_global[0, 1, :]) == 0.0
        ):
            ...
            if (curr_wp[:2] != prev_wp[:2]).all():
                ...
                slope_waypoint = (curr_wp[1] - prev_wp[1]) / (
                    curr_wp[0] - prev_wp[0]
                )  # this is m
    ...
    for i in range(waypoints_global.shape[0]):
        test_ego_points = position_to_ego_frame(
            ego_global["position"], ego_global["position"], ego_global["heading"]
        )
        waypoints_local[i] = position_to_ego_frame(
            waypoints_global[i], ego_global["position"], ego_global["heading"]
        )
    ...
    for i in range(waypoints_pix_local.shape[0]):
        waypoints_img[i] = waypoints_pix_local[i, :2] - np.array(
            [-64, 64]
        )  # translation
    ...
    for i in range(waypoints_img.shape[0]):
        patch = gaussian_heatmap(
            np.array([waypoints_img[i][0], waypoints_img[i][1]]), sig=2
        )
        route_map = route_map + patch

    if route_map.max() > 0:
        route_map = (
            route_map / route_map.max() * 255
        )  # scale between 0 to 255 to match standard image space
