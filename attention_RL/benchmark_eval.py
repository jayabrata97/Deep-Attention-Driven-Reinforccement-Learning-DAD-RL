...
def benchmark_score(
    ...
    if scenario_path == "../SMARTS/scenarios/sumo/SRT/cross":
    ...
    while num_resets < episodes:
        ...
        if done:
            ...
            mean_progress = (
                mean_progress * (num_resets - 1) + ep_progress
            ) / num_resets
            ...
            mean_off_road = (
                mean_off_road * (num_resets - 1) + ep_off_road
            ) / num_resets
            mean_off_route = (
                mean_off_route * (num_resets - 1) + ep_off_route / i
            ) / num_resets  # normalized frequency of off_route in an episode(0-1)
            mean_wrong_way = (
                mean_wrong_way * (num_resets - 1) + ep_wrong_way / i
            ) / num_resets  # normalized frequency of wrong_way in an episode(0-1)
            mean_not_moving = (
                mean_not_moving * (num_resets - 1) + ep_not_moving / i
            ) / num_resets  # normalized frequency of not_moving in an episode(0-1)
    ...
    mean_reward_components = np.array(
        [
            mean_progress,
            mean_speed,
            mean_goal,
...
def benchmark_logger(
    ...
    for j, (scenario, eval_seed) in enumerate(zip(eval_scenarios, eval_seeds)):
        Score[j], Mean_length[j], Mean_reward[j], Rew_components[j], _, _ = (
            benchmark_score(
                model_path,
                scenario,
                eval_seed,
    ...
    for j, (score, mean_length, mean_reward, rew_components) in enumerate(
        zip(Score, Mean_length, Mean_reward, Rew_components)
    ):
    ...
    writer.add_scalar(
        f"Benchmark_Combined/Overall Score",
        total_score_overall / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Benchmark_Combined/Distance to destination",
        total_score_d2d / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Benchmark_Combined/Humanness error",
        total_score_he / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Benchmark_Combined/Rule Violation",
        total_score_rv / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Benchmark_Combined/Time", total_score_time / N_eval_envs, i * TIMESTEPS
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_ep_length", total_length / N_eval_envs, i * TIMESTEPS
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_reward", total_reward / N_eval_envs, i * TIMESTEPS
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_progress",
        total_rew_components[0] / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_speed",
        total_rew_components[1] / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_goal", total_rew_components[2] / N_eval_envs, i * TIMESTEPS
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_crash",
        total_rew_components[3] / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_off_road",
        total_rew_components[4] / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_off_route",
        total_rew_components[5] / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_wrong_way",
        total_rew_components[6] / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.add_scalar(
        f"Eval_Combined/Mean_not_moving",
        total_rew_components[7] / N_eval_envs,
        i * TIMESTEPS,
    )
    writer.close()
